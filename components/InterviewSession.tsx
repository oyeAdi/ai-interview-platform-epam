        ...
        <div className="fixed inset-0 z-[100] h-screen w-screen bg-[#F8FAFC] overflow-hidden flex font-sans text-slate-900">
            ...
            <div className="w-[45%] h-full bg-[#F1F5F9] border-l border-slate-200 flex flex-col overflow-hidden">
                <div className="flex-1 flex flex-col p-8 space-y-6 overflow-y-auto">
                    ...
                    <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
                        <div className="flex items-center justify-between mb-4">
                            <h3 className="text-xs font-black text-[#0095A9] uppercase tracking-widest">
                                {currentRound === 'MCQ' ? 'Round 1: Screening' :
                                    currentRound === 'CONCEPTUAL' ? 'Round 2: Conceptual' :
                                        currentRound === 'CODING' ? 'Round 3: Practical Coding' : 'Round 4: System Design'}
                            </h3>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-center justify-between text-sm">
                                ...
                                <span className="font-black text-slate-700">
                                    {(currentRound === 'MCQ' || currentRound === 'CONCEPTUAL') ? (
                                        ...
                                        <>Challenge {codingChallengeCount} of {currentRound === 'CODING' ? (config?.coding?.maxQuestions || 1) : 1}</>
                                    )}
                                </span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                                <div
                                    ...
                                    style={{
                                        width: `${((currentRound === 'MCQ' || currentRound === 'CONCEPTUAL' ? questionCount : codingChallengeCount) /
                                            (currentRound === 'MCQ' ? (config?.mcq?.maxQuestions || 7) :
                                                currentRound === 'CONCEPTUAL' ? (config?.conceptual?.maxQuestions || 10) :
                                                    currentRound === 'CODING' ? (config?.coding?.maxQuestions || 1) :
                                                        ((config as any)?.systemDesign?.maxQuestions || 1))) * 100}%`
                                    }}
                                />
                            </div>
                        </div>
                    ...
                    {(currentRound === 'CODING' || currentRound === 'SYSTEM_DESIGN') ? (
                        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100 space-y-6">
                            <div className="flex items-center gap-2 pb-4 border-b border-slate-50">
                                <AlertCircle className="w-5 h-5 text-[#0095A9]" />
                                <h3 className="text-xs font-black text-slate-700 uppercase tracking-widest">Instructions</h3>
                            </div>
                            ...
                            <div className="space-y-4">
                                <p className="text-sm text-slate-600 leading-relaxed">
                                    {currentRound === 'CODING' ? (
                                        <>
                                            Please use your favorite local IDE (VS Code, IntelliJ, etc.) or an online compiler to solve this challenge.
                                            <strong> Copy the boilerplate class</strong> from the question, implement your logic, and paste <strong>only the solution part</strong> back here in the chat.
                                        </>
                                    ) : (
                                        <>
                                            Utilize any design tools or local editors to draft your architecture.
                                            Submit your design description, component breakdowns, or trade-off analysis here in the chat once ready.
                                        </>
                                    )}
                                </p>

                                {currentRound === 'CODING' && (
                                    <div className="bg-slate-50 rounded-xl p-4 font-mono text-[11px] text-slate-500 border border-slate-100">
                                        <div className="flex items-center justify-between mb-2">
                                            <span className="uppercase tracking-widest font-black text-[9px] text-slate-400">Example Format</span>
                                        </div>
                                        <pre className="text-slate-600">
                                            {`class Solution {\n    public long maximumSum(int[] arr) {\n        // Your implementation here\n    }\n}`}
                                        </pre>
                                    </div>
                                )}

                                <div className="flex items-start gap-3 p-4 bg-[#0095A9]/5 rounded-xl border border-[#0095A9]/10">
                                    <ShieldCheck className="w-5 h-5 text-[#0095A9] flex-shrink-0" />
                                    <p className="text-[11px] text-[#0095A9] font-medium leading-relaxed">
                                        Note: Your screen is being recorded for integrity. DM will review your local development process.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-gradient-to-br from-[#0095A9]/5 to-[#00B4CC]/5 rounded-2xl p-6 border border-[#0095A9]/10">
                        ...
                        <p className="text-sm text-slate-600 leading-relaxed">
                            {currentRound === 'MCQ' ? "Take your time to read each question carefully." :
                                currentRound === 'CONCEPTUAL' ? "Explain your thought process clearly." :
                                    currentRound === 'CODING' ? "Ensure your code is clean and handles edge cases." :
