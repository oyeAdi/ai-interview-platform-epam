...
export default function AssessmentShield({ children, onViolation, onTerminate, sessionId, disabled = false }: AssessmentShieldProps) {
    ...
    // 1. Multi-Tab Detection with delayed handshake
    useEffect(() => {
        ...
        channel.onmessage = (event) => {
            ...
            } else if (event.data === 'pong' && !receivedPong) {
                receivedPong = true;
                setIsMultiTab(true);
                handleViolation('MULTI_TAB_DETECTED', 'Session already open in another tab');
            }
        };

        // Delay ping to allow other tabs to set up listeners
    ...
    useEffect(() => {
        const detectDevTools = () => {
            ...
            if (widthThreshold || heightThreshold) {
                handleViolation('DEVTOOLS_DETECTED', 'Browser developer tools may be open');
            }
        };
    ...
    // 4. Fullscreen Listeners (Fixed Race Condition & Side Effects)
    useEffect(() => {
        const handleFullscreenChange = () => {
            ...
            isFullscreenRef.current = currentFullscreen;

            // Only increment strikes if we HAD fullscreen and NOW we don't
            if (prevFullscreen && !currentFullscreen) {
    ...
    return (
        <div ref={containerRef} className="relative w-full h-screen bg-white">
            ...
            {!isFullscreen && !isMultiTab && !disabled && (
                            ...
                            {fullscreenStrikes > 0 ? (
                                <div className="p-4 bg-red-500/10 border border-red-500/20 rounded-2xl">
                                    ...
                                    <p className="text-sm text-red-200">
                                        One more exit will result in **immediate termination** of your interview.
                                    </p>
                                </div>
                            ) : (
